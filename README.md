...
tools/
  calibrate_skin_matrix.py # spectral demo → matrix/mask JSON
```
...
`arri_skin_d55` preset is generated by `tools/calibrate_skin_matrix.py` from demo spectral
data: rough ALEV3 / IMX410 curves, a D55 illuminant, and a sigmoid Sigma fp hot-mirror
assumption. Runtime export only reads `dngscan/scene_transform_presets.json`; it does not
need `colour-science` or `scipy`.
...
For serious calibration, pass digitized ALEV3 SSF, IMX410 QE and real skin reflectance CSVs
to the script. The built-in curves are placeholders intended to prove the chain:
