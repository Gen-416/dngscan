...
GUI 在 localhost 运行，完全离线。选中文件后会静默预热代理 scene 与基础分析；同一文件
的代理缓存会留在内存，并写入本机用户缓存，重启后再次打开也可直接复用。macOS 默认位置
是 `~/Library/Caches/dngscan/preview-v1`，最多 768 MB，自动淘汰旧条目；删除它只会让下次
重新预热，不会影响 RAW 或成片。预览走代理图，导出始终回到全分辨率场景缓冲，并在一次性
工作进程中完成，结束后全尺寸内存会交还系统。合理的第一次使用：EV 0、默认 AgX 核心打开一张 RAW，
