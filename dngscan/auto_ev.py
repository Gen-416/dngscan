...
def output_highlight_margin(rgb_linear: Any, gamut: str) -> float:
    ...
    margin_luma = 92.0 - y_p999 * 100.0
    margin_rgb = 96.0 - max_p999 * 100.0
    margin_clip = 0.03 - float(np.mean(np.any(rgb >= np.float32(0.999), axis=1)) * 100.0)
    margin_near = 0.25 - float(np.mean(max_channel >= np.float32(0.956)) * 100.0)
...
def max_safe_ev(
    ...
    def margin_at(ev: float) -> float:
        ...
        return output_highlight_margin(rgb, gamut)

    try:
        if margin_at(from_ev) <= 0.0:
            return float(from_ev)
...
def compute_auto_ev(
    ...
    """Align scene median to 18% gray; cap upward boost by output highlight safety."""
